...
```bash
itop_nexmo$ sam build --use-container
```
...
```bash
itop_nexmo$ sam logs -n StockCheckerFunction --stack-name itop_nexmo --tail
```
...
```bash
itop_nexmo$ pip install pytest pytest-mock --user
itop_nexmo$ python -m pytest tests/ -v
```
...
```bash
aws cloudformation delete-stack --stack-name itop_nexmo
```
